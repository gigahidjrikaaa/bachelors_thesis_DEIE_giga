...
To operationalize the BDI model, the decision-making logic of the Safety Agent Suite is formalized as a set of mapping functions. This formalization clarifies how the Aika Meta-Agent orchestrates the specialized agents based on user inputs and context, bridging the theoretical framework to the practical implementation in Chapter 3.

\paragraph{Aika Meta-Agent (Orchestrator)}
As the primary interface, Aika performs the initial intent classification ($I$) and Tier 1 risk assessment ($R_{immediate}$) on the current user message ($m_t$):
\begin{equation}
(I, R_{immediate}) = f_{Aika}(m_t, \text{UserRole})
\end{equation}
This output determines the immediate routing logic:
\begin{equation}
...
Operating in the background, the STA performs a deep, Tier 2 analysis on the full conversation history ($H_t$) to assess the cumulative risk level ($R_{cumulative}$):
\begin{equation}
R_{cumulative} = f_{STA}(H_t)
\end{equation}
Unlike Aika's immediate check, this function analyzes patterns across multiple turns to identify latent distress that may not be apparent in a single message.

\paragraph{Therapeutic Coach Agent (TCA)}
If the STA identifies moderate distress, the TCA is invoked to generate a personalized intervention plan ($P_{intervention}$), rather than a direct chat response:
\begin{equation}
P_{intervention} = f_{TCA}(H_t, \text{UserProfile})
\end{equation}
This plan is stored asynchronously for the student to access, ensuring that therapeutic content is delivered without disrupting the conversational flow.

\paragraph{Case Management Agent (CMA)}
The CMA is triggered for administrative or crisis actions ($\alpha$), such as scheduling appointments or creating crisis tickets, based on the current case state ($C_t$) and resource availability ($Res$):
\begin{equation}
\alpha = f_{CMA}(C_t, Res)
\end{equation}

\paragraph{Insights Agent (IA)}
The IA aggregates anonymized data ($\mathcal{D}$) to compute population-level metrics ($\mu$) while enforcing privacy constraints ($k$-anonymity):
\begin{equation}
\mu = f_{IA}(\mathcal{D}, \text{Query}) \quad \text{s.t. } \text{Privacy}(\mu) \ge k
\end{equation}

These formalizations establish the logical foundation for the multi-agent coordination, ensuring that each agent operates within a clearly defined scope while contributing to the cohesive system behavior.
...
\end{equation}
Here, $S_t$ represents the \textbf{Global State Object} at time step $t$, which contains the complete conversation history and metadata. $\Delta S_i$ is the \textbf{State Update} produced by node $i$ (e.g., a new message or a risk classification). The operator $\oplus$ denotes the \textbf{State Merging Operation}, where new fields in $\Delta S_i$ overwrite or append to existing values in $S_t$ while preserving all other unmodified fields. This ensures that context is never lost as the workflow progresses.
