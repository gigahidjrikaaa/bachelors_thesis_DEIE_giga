...
For orchestration reliability, a test suite of 10 structured conversation flows was designed to exercise various paths through the agent system, including successful triage-to-coaching handoffs, escalations to case management, and error handling. Each scenario was executed, and the system's behavior was logged via Langfuse. The success criteria were a tool call success rate of 95\% or higher and 100\% state transition accuracy.

For intervention quality, the Therapeutic Coach Agent (TCA) was tasked with generating responses to 10 coaching scenarios covering common student issues (e.g., academic stress, motivation). These responses were evaluated using a \textbf{Rubric-Based Human Evaluation} methodology. The primary researcher scored each response against a 5-point rubric (see Appendix~\ref{app:coaching_prompts}) that assessed empathy, appropriateness, and adherence to basic CBT principles. Additionally, an automated \textbf{LLM-as-a-Judge} evaluation was conducted using \textbf{Sherlock Think Alpha} as an independent validator. The success criterion was an average rubric score of 3.5 or higher.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{l c}
        ...
        \midrule
        Overall Tool-Call Success Rate & 98.2\% \\
        State Transition Accuracy & 100.0\% \\
        Retry and Recovery Success Rate & 100.0\% \\
...
\subsection{Discussion}
This section will discuss the robustness of the state graph and the quality of the therapeutic content. It will analyze any orchestration failures and discuss the strengths of the TCA's empathetic responses, linking them to the prompt engineering strategies employed.
...
\subsection{Discussion}
This section will confirm that the privacy mechanisms are implemented correctly, forming a critical safeguard for the system. It will discuss the implications of k-anonymity for data utility and potential future enhancements like differential privacy.
