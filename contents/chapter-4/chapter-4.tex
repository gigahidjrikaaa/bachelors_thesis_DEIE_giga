...
\begin{table}[htbp]
    ...
    \begin{tabular}{p{1.6cm}p{2.8cm}p{3.0cm}p{2.8cm}p{2.0cm}}
        ...
        RQ3 (Quality) & 10 coaching prompts scored via structured rubric by researcher + GPT-4 & CBT adherence, empathy, appropriateness, actionability (1-5 scale) & Mean scores $\geq 3.5$, researcher-GPT-4 agreement $\geq 0.70$ & \S\ref{sec:rq3} \\
...
\textbf{Calculation Procedure:}
\begin{enumerate}
    \item Generate SCA responses for all 25 coaching prompts via \lstinline {/api/v1/aika} endpoint
    \item Primary researcher scores each response using structured rubric
    \item Record scores in spreadsheet with columns: \lstinline {prompt\_id, cbt\_score, empathy\_score, appropriateness\_score, actionability\_score}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item Verify all expected agent nodes appear in trace (STA, SCA, SDA, IA as per scenario)
        \item Confirm state transitions follow expected workflow (e.g., F1: message → STA node → escalation decision → SCA node → coaching response)
...
\textbf{Objective:} Evaluate the appropriateness, empathy, and CBT-alignment of Support Coach Agent responses through structured qualitative assessment with dual-rater validation (researcher + GPT-4).
...
\begin{enumerate}
    \item \textbf{Response Generation:} For each of 10 prompts, send to \texttt{/api/v1/aika} endpoint with conversation context. Save SCA response to structured spreadsheet with columns: \texttt{\{prompt\_id, prompt\_text, sca\_response, notes\}}.
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        \node[process, right=of prompts] (generate) {Generate SCA\\Responses};
...
\begin{table}[htbp]
    ...
    \begin{tabular}{lp{8cm}}
        ...
        \midrule
        \texttt{test\_suppress\_small\_cohort} & Query with cohort size = 3 returns empty result (automatic suppression) \\
        \texttt{test\_publish\_compliant\_cohort} & Query with cohort size = 8 returns aggregate data (passes k-anonymity) \\
        \texttt{test\_block\_individual\_query} & Attempt to query single user's data raises \texttt{PrivacyViolationError} \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item \textbf{Setup:} Create mock database with 3 conversations from 3 distinct users discussing "exam stress"
        \item \textbf{Execute:} Run \texttt{topic\_distribution} query for "exam stress" topic
        \item \textbf{Assert:} Query returns empty list (no rows), confirming automatic suppression for $n < 5$
        \item \textbf{Code Location:} \texttt{backend/tests/test\_insights\_privacy.py::test\_suppress\_small\_cohort}
    \end{itemize}
    
    \item \textbf{Test 2: Compliant Cohort Publication}
    \begin{itemize}
        \item \textbf{Setup:} Create mock database with 8 conversations from 8 distinct users discussing "relationship issues"
        \item \textbf{Execute:} Run \texttt{topic\_distribution} query for "relationship issues"
        \item \textbf{Assert:} Query returns 1 row with aggregate: \texttt{\{topic: "relationship issues", count: 8, avg\_sentiment: X\}}
        \item \textbf{Code Location:} \texttt{backend/tests/test\_insights\_privacy.py::test\_publish\_compliant\_cohort}
    \end{itemize}
    
    \item \textbf{Test 3: Individual Query Blocking}
    \begin{itemize}
        \item \textbf{Setup:} Attempt to construct SQL query requesting data for specific \texttt{user\_id=12345}
        \item \textbf{Execute:} Call \texttt{InsightsAgentService.execute\_query()} with individual user filter
        \item \textbf{Assert:} Service raises \texttt{PrivacyViolationError} with message: "Individual-level queries are not permitted"
        \item \textbf{Code Location:} \texttt{backend/tests/test\_insights\_privacy.py::test\_block\_individual\_query}
    \end{itemize}
    ...
    \item \textbf{Success Metric:} All 3 tests must pass (100\% pass rate).
\end{enumerate}
...
Key findings:
\begin{itemize}
    \item \textbf{What worked:} Standard conversation flows (STA→SCA, SCA-only, IA queries) executed without errors. State management correctly preserved conversation context across agent handoffs. Tool invocations (database queries, case creation) succeeded with proper input/output capture.
...
\begin{itemize}
    ...
    \item \textbf{Human-AI Collaboration Design:} Explicit integration points for human oversight (STA low-confidence review, SCA escalation pathways, IA expert interpretation) reflect realistic deployment constraints absent in academic multi-agent simulations.
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Monolithic vs. Specialized Agents:} Existing chatbots lack role specialization for safety triage vs. therapeutic support. → \textit{Addressed:} Multi-agent architecture (STA, SCA, SDA, IA) with independent evaluation per agent (RQ1-RQ4).
...
\begin{enumerate}
    ...
    \item \textbf{Robust Human Handoff:} Design SCA refusal responses (out-of-scope requests) with explicit contact information: "I cannot provide medical advice, but I can connect you with [University Health Center: phone, hours, location]". Include warm handoff protocols where agent facilitates appointment booking rather than providing passive referrals.
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{SCA Quality Monitoring:} Random sample of 5\% of coaching conversations flagged for human quality review. Counselors score using same rubric (Table~\ref{tab:rq3_rubric}), enabling continuous quality tracking and prompt engineering feedback loops.
\end{itemize}
