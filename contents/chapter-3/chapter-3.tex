...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture} [
        ...
        activity/.style={rectangle, draw=ugmBlue, very thick, rounded corners=3pt, fill=ugmBlue!5, minimum width=3.5cm, minimum height=1cm, align=center, font=\small},
        arrow/.style={-Latex, thick, ugmBlue},
        feedback/.style={-Latex, thick, ugmGold, dashed}
    ]
        % Nodes for each stage
        \node[activity] (problem) {1. Problem Identification \& Motivation};
        \node[activity, right=of problem] (objectives) {2. Define Objectives};
        \node[activity, right=of objectives] (design) {3. Design \& Development};
        \node[activity, below=of design] (evaluation) {5. Evaluation};
        \node[activity, left=of evaluation] (demonstration) {4. Demonstration};
        \node[activity, left=of demonstration] (communication) {6. Communication};

        % Arrows for the main flow
        \draw[arrow] (problem) -- (objectives);
        \draw[arrow] (objectives) -- (design);
        \draw[arrow] (design) -- (evaluation);
        \draw[arrow] (evaluation) -- (demonstration);
        \draw[arrow] (demonstration) -- (communication);
        
        % Feedback loops
        \draw[feedback] (design.south) to[out=-45, in=45] (objectives.south);
        \draw[feedback] (evaluation.east) to[out=0, in=0, looseness=1.5] (design.east);
        \draw[feedback] (demonstration.west) to[out=180, in=180, looseness=1.5] (design.west);
        \draw[feedback] (communication.north) to[out=135, in=225, looseness=1.5] (problem.west);

    \end{tikzpicture}%
    }% End resizebox
    \caption{The Design Science Research (DSR) process model as applied in this thesis, adapted from Peffers et al. \cite{dsr_methodology_peers_2006}. The diagram shows the sequential stages and the iterative feedback loops that inform the research process.}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{p{2.5cm} p{10.5cm}}
        ...
        \textbf{Safety Triage Agent (STA)} & A conceptual role embedded within Aika's initial analysis. Classifies immediate (Tier 1) and conversational (Tier 2) risk to enforce safety protocols. \\
...
The synergy between these two loops is the cornerstone of the framework's design. The real-time loop gathers the data that fuels the strategic loop, while the insights from the strategic loop can be used to configure and improve the proactive interventions delivered by the real-time loop, creating a continuously learning and adaptive support ecosystem. This dual-loop architecture is visualized in Figure \ref{fig:two_loops_diagram}.
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        data_arrow/.style={-Latex, thick, dashed}
    ]
        % Real-Time Interaction Loop
        \node[loop=The Real-Time Interaction Loop] (realtime_loop) {};
        \node[actor, left=of realtime_loop] (student) {Student};
        \node[agent, at=(realtime_loop.center)] (agents) {Aika, STA, \\ TCA, CMA};
        
        \draw[arrow] (student) to[bend left=20] node[midway, above, font=\small] {Interaction} (agents);
        \draw[arrow] (agents) to[bend left=20] node[midway, below, font=\small] {Support} (student);

        % Strategic Oversight Loop
        \node[loop=The Strategic Oversight Loop, below=4.5cm of realtime_loop] (strategic_loop) {};
        \node[actor, right=of strategic_loop] (admin) {Admin / \\ Counselor};
        \node[agent, at=(strategic_loop.center)] (ia) {Insights Agent \\ (IA)};
        
        \draw[arrow] (ia) to[bend left=20] node[midway, above, font=\small] {Reports} (admin);
        \draw[arrow] (admin) to[bend left=20] node[midway, below, font=\small] {Queries} (ia);

        % Connecting the loops
        \node[data] (agg_data) at ([yshift=-2.25cm]realtime_loop.center) {Aggregated, Anonymized Data};
        
        \draw[data_arrow] (agents) -- (agg_data) node[midway, right, font=\small] {Fuels};
        \draw[data_arrow] (agg_data) -- (ia) node[midway, right, font=\small] {Informs};
        \draw[data_arrow] (ia) to[bend right=30] (agents) node[midway, left, font=\small] {Improves};

    \end{tikzpicture}%
    }
    \caption{The Two Proactive Loops: The real-time loop provides immediate student support, generating data that fuels the strategic loop. The strategic loop analyzes this data to produce insights that, in turn, improve the proactive capabilities of the real-time loop.}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{p{3.5cm} p{1.5cm} p{8cm}}
        ...
        \midrule
        \code{reply} & string & The empathetic, context-aware conversational response to be displayed to the user. \\
        \code{immediate\_risk} & string & A five-level risk classification (\texttt{none}, \texttt{low}, \texttt{moderate}, \texttt{high}, \texttt{critical}) for the single message, enabling instantaneous safety screening. \\
        ...
        \code{intent} & string & The classified user intent (e.g., \texttt{request\_support}, \texttt{schedule\_appointment}), which dictates subsequent routing logic. \\
        ...
        \code{suggested\_response} & string & An optional, complete response for simple interactions, allowing the system to bypass agent orchestration and reduce latency. \\
        \bottomrule
    \end{tabular}
\end{table}

This unified response schema yields several architectural benefits. First, it facilitates \textbf{latency optimization}; by consolidating response generation and risk assessment into one call, the system can achieve sub-300ms response times, which is critical for maintaining conversational fluidity. Second, it enables \textbf{embedded safety}, as risk assessment is an integral and non-negotiable part of every interaction loop. Third, the schema ensures \textbf{transparent oversight} by providing a clear audit trail for the system's reasoning. Finally, the \code{needs_agents} flag allows for \textbf{conditional agent invocation}, an efficient resource management strategy that reduces backend compute costs by bypassing complex orchestration for simple queries.

An example of this schema in practice is shown in Listing~\ref{lst:json_moderate_stress}, where a user expresses moderate, non-imminent distress. In contrast, Listing~\ref{lst:json_greeting} shows the optimized response for a simple greeting. This structure operationalizes the principle that Aika is the sole user-facing component, synthesizing conversational intelligence and safety screening into a single, coherent interface layer.

\begin{lstlisting}[style=jsonStyle,caption={Example Aika JSON response for moderate stress scenario},label={lst:json_moderate_stress}]
{
  ...
  "crisis_keywords": ["ujian", "stres"],
  "risk_reasoning": "User expresses feelings of pressure and anxiety related to upcoming exams, indicating moderate, non-imminent distress.",
  "intent": "request_emotional_support",
  "needs_agents": true,
...
\begin{lstlisting}[style=jsonStyle,caption={Example Aika JSON response for casual greeting},label={lst:json_greeting}]
{
  ...
  "immediate_risk": "none",
  "crisis_keywords": [],
  "risk_reasoning": "Casual greeting with no indicators of distress.",
  "intent": "casual_greeting",
  "needs_agents": false,
  "reasoning": "Simple greeting, no specialist agent required.",
...
\begin{itemize}
    \item \textbf{Beliefs:} The STA's beliefs are formed from the raw text of the user's message and the structured output of the Gemini 2.5 Flash model, which provides a risk classification (\texttt{low}, \texttt{moderate}, \texttt{critical}) and detected crisis keywords.
    \item \textbf{Desires:} Its fundamental desire is to ensure user safety by correctly identifying and escalating any potential crisis.
    \item \textbf{Intentions:} Based on its beliefs, the STA forms a clear intention. If the belief is that the risk is \texttt{critical}, its intention becomes to update the agent state to trigger the "escalate\_to\_cma" edge in the LangGraph. Otherwise, its intention is to pass control to the next appropriate agent.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Beliefs:} The TCA's beliefs include the user's message history, the STA's "safe" classification, and any previously generated intervention plans stored in the agent state.
    \item \textbf{Desires:} Its core desire is to reduce user distress and build coping skills by providing helpful, evidence-based guidance.
    \item \textbf{Intentions:} If the user's message indicates a need for coping strategies, the TCA forms the intention to execute its \texttt{generate\_intervention\_plan} tool. This tool call is a committed action to fulfill its desire. If the user is simply venting, its intention is to generate an empathetic, listening-oriented response.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Beliefs:} The CMA believes the state of the world requires administrative action. This belief is triggered by either a \texttt{critical} flag from the STA or a direct user request for a service like booking an appointment.
    \item \textbf{Desires:} Its primary desire is to execute administrative workflows reliably and accurately.
    \item \textbf{Intentions:} When triggered by a crisis escalation, it forms the intention to execute the \texttt{create\_crisis\_case} tool. When triggered by a user request, it forms the intention to use the \texttt{schedule\_appointment} tool. Each intention maps directly to a deterministic, procedural tool call.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Agent Orchestration: LangGraph.} The complex, conditional logic of the multi-agent system is managed using LangGraph. LangGraph provides a stateful, graph-based framework for composing agents. This is a significant improvement over stateless LLM calls, as it allows the system to maintain a coherent state across multiple turns of a conversation and multiple agent invocations. It directly enables the implementation of the agentic loops and decision points described in the functional architecture.
...
The heart of the technical architecture is the LangGraph state machine, which operationalizes the agentic behavior. The orchestration is not a simple linear chain but a cyclical graph where the flow is determined by the contents of a shared \texttt{AgentState} object.

The process is as follows:
\begin{enumerate}
    \item A user message initializes the \texttt{AgentState}.
    \item The graph routes the state to the first node, the \textbf{Safety Triage Agent (STA)}.
    \item The STA executes its logic and updates the \texttt{AgentState} with a risk assessment and a routing decision (e.g., \texttt{next\_step: "tca"}).
    \item A conditional edge reads the \texttt{next\_step} from the state and routes it to the appropriate next node, which could be the \textbf{Therapeutic Coach Agent (TCA)}, the \textbf{Case Management Agent (CMA)}, or the end of the graph.
    \item This process repeats, with each agent modifying the shared state, until a terminal node is reached.
\end{enumerate}
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        transform shape,
        node distance=2.5cm and 1.5cm,
        agent/.style={rectangle, draw, fill=ugmBlue!10, rounded corners, thick, minimum height=1.2cm, minimum width=2.8cm, text centered, align=center, font=\small},
        meta_agent/.style={rectangle, draw, fill=ugmGold!20, rounded corners, thick, minimum height=1.5cm, minimum width=3.5cm, text centered, align=center, font=\small\bfseries},
        decision/.style={diamond, draw, fill=gray!10, aspect=1.5, inner sep=0pt, minimum size=1.5cm, text centered, font=\scriptsize},
        io/.style={trapezium, trapezium left angle=70, trapezium right angle=110, draw, fill=gray!5, minimum height=1cm, text centered, align=center, font=\small},
        arrow/.style={-Stealth, thick},
        line/.style={thick}
    ]

        % Nodes
        \node[io] (user_input) {User Message};
        \node[meta_agent, right=of user_input] (meta_agent) {Aika Meta-Agent \\ (Orchestrator)};
        \node[agent, below=of meta_agent] (sta) {Safety Triage Agent \\ (STA)};
        \node[decision, below=of sta] (risk_decision) {Risk Level?};
        
        \node[agent, right=of risk_decision] (cma) {Case Management Agent \\ (CMA)};
        \node[agent, left=of risk_decision] (tca) {Therapeutic Coach Agent \\ (TCA)};
        
        \node[io, below=of tca] (tca_output) {Therapeutic Response};
        \node[io, below=of cma] (cma_output) {Escalation \& \\ Counselor Notification};
        \node[io, below=of risk_decision, yshift=-0.5cm] (low_risk_output) {Standard Response};

        % Edges
        \draw[arrow] (user_input) -- (meta_agent);
        \draw[arrow] (meta_agent) -- (sta);
        \draw[arrow] (sta) -- (risk_decision);

        \draw[arrow] (risk_decision) -- node[above, font=\scriptsize] {Low} (low_risk_output);
        \draw[arrow] (risk_decision) -- node[above, font=\scriptsize] {Moderate} (tca);
        \draw[arrow] (risk_decision) -- node[above, font=\scriptsize] {Critical} (cma);
        
        \draw[arrow] (tca) -- (tca_output);
        \draw[arrow] (cma) -- (cma_output);

        % Feedback loops to Meta-Agent for synthesis (optional, for clarity)
        \draw[arrow, dashed] (tca_output) -| ([xshift=-1.5cm]meta_agent.south) node[pos=0.95, left, font=\tiny] {Synthesize};
        \draw[arrow, dashed] (cma_output) -| ([xshift=1.5cm]meta_agent.south) node[pos=0.95, right, font=\tiny] {Synthesize};
        \draw[arrow, dashed] (low_risk_output) -- (meta_agent.south) node[pos=0.8, right, font=\tiny] {Synthesize};

    \end{tikzpicture}
    \caption{High-level visualization of the LangGraph agent orchestration state machine. The Aika Meta-Agent routes user input to the STA for risk assessment, then conditionally invokes the TCA for therapeutic coaching or the CMA for crisis escalation based on the outcome.}
...
\begin{enumerate}
    \item The \textbf{STA} detects a message with "Critical" risk.
