...
\begin{enumerate}
    ...
    Based on this initial assessment, Aika acts as a dispatcher to the background specialist agents:
    \begin{itemize}
        \item \textbf{Therapeutic Coach Agent (TCA):} Triggered by Aika when the risk is assessed as \texttt{moderate} or \texttt{low}. It works in the background to generate CBT-based intervention plans.
        \item \textbf{Case Management Agent (CMA):} Triggered directly by Aika when the user explicitly requests administrative actions (e.g., scheduling). It is also triggered by the STA if a critical risk is detected.
        \item \textbf{Safety Triage Agent (STA):} Operates primarily in the background. It is triggered by the Aika Meta-Agent when the initial Tier 1 screening detects ambiguous safety signals or requires a deeper, context-aware analysis (Tier 2).
    \end{itemize}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{p{3.5cm} p{1.5cm} p{8cm}}
        ...
        \code{intent} & string & The classified user intent (e.g., \code{request\_support}, \code{schedule\_appointment}), which dictates subsequent routing logic. \\
...
\begin{lstlisting}[style=academicStyle, language=json]
{
  "reply": "Halo! Saya Aika, asisten kesehatan mentalmu. Ada yang bisa saya bantu hari ini?",
  "immediate_risk": "none",
  "crisis_keywords": [],
...
The Safety Triage Agent (STA) serves as the system's comprehensive safety monitor. Unlike Aika's immediate Tier 1 screening, the STA performs deep-dive, asynchronous analysis (Tier 2). It is triggered by the Aika Meta-Agent when the initial Tier 1 screening detects ambiguous safety signals or requires a deeper, context-aware analysis (Tier 2). While it can operate in the background to analyze full conversation histories, its primary role in the real-time loop is to provide a rigorous safety check for inputs that Aika flags as potentially risky but not immediately critical.
...
The heart of the technical architecture is the LangGraph state machine, which operationalizes the agentic behavior. The orchestration follows a "dispatcher" pattern where the Aika Meta-Agent serves as the central decision node, routing control to specialist agents only when specific conditions are met.
...
\begin{enumerate}
    ...
    \item Aika analyzes the input using its system prompt and updates the \texttt{AgentState} with a Tier 1 risk assessment and a routing decision (e.g., \texttt{needs\_agents: true}, \texttt{next\_step: "sta"}).
    \item A conditional edge reads the \texttt{next\_step} from the state and routes it to the appropriate next node:
    \begin{itemize}
        \item \textbf{Safety Triage Agent (STA):} For background risk analysis (Tier 2).
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        agent/.style={rectangle, draw=black, thick, fill=white, text width=2.8cm, align=center, rounded corners=2pt, minimum height=1.2cm, font=\small\bfseries},
        ...
        \node [agent, below=of start] (aika) {Aika Meta-Agent\\(Dispatcher)};
        
        % Specialist Agents arranged below Aika
        \node [agent, below left=of aika, xshift=-0.5cm, yshift=-1cm] (sta) {Safety Triage\\(STA)};
        \node [agent, below=of aika, yshift=-1cm] (tca) {Therapeutic Coach\\(TCA)};
        \node [agent, below right=of aika, xshift=0.5cm, yshift=-1cm] (cma) {Case Mgmt\\(CMA)};
        
        \node [startstop, below=of tca, yshift=-1cm] (end) {End};
        ...
        \path [line] (aika) -- node [label_node] {Coaching} (tca);
        \path [line] (aika) -| node [label_node, near start] {Admin/Crisis} (cma);
        
        % Direct Reply path (curved to avoid crossing)
        \path [line] (aika.east) -- ++(1.5,0) |- node [label_node, near start] {Direct Reply} (end);
    ...
    \end{tikzpicture}
    \caption{LangGraph State Machine Visualization. Aika acts as the central dispatcher, routing the conversation to specialist agents (STA, TCA, CMA) or responding directly based on the context.}
