...
\begin{table}[htbp]
    ...
    \begin{tabular}{p{3.2cm}p{2.5cm}p{3.5cm}p{3.3cm}}
        ...
        \midrule
        Privacy validation (RQ4) & Code review + 3 unit tests & Inspection of 6 allow-listed IA SQL queries for k-anonymity enforcement (\texttt{HAVING COUNT(DISTINCT user\_id) >= 5}). Unit tests: small cohort suppression, compliant publication, individual query blocking. & Insights Agent (IA): k-anonymity implementation correctness. Validates privacy safeguards function as designed without requiring synthetic log generation. \\
        \bottomrule
    \end{tabular}
\end{table}
...
To enable comprehensive evaluation of the multi-agent framework (detailed in Chapter~\ref{chap:evaluation}), the backend service incorporates a production-grade observability stack. This infrastructure serves dual purposes: operational monitoring during development and systematic data collection for research question evaluation.

The monitoring architecture consists of three integrated components designed to provide complete visibility into agent behavior and system performance:

\paragraph{Prometheus Metrics Collection.} The backend exposes a \texttt{/metrics} endpoint implementing the Prometheus exposition format \cite{PLACEHOLDER_CITE_prometheus}. Custom metrics instruments track critical performance indicators across three dimensions:

\begin{itemize}
    \item \textbf{Agent Performance Metrics:} Processing time histograms (\texttt{agent\_processing\_time\_seconds}), success rates by agent type (\texttt{agent\_success\_rate}), and invocation counters (\texttt{agent\_invocations\_total}). These metrics enable quantitative evaluation of RQ2 (orchestration correctness and latency).
    \item \textbf{Tool Execution Metrics:} Tool call counters with success/failure labels (\texttt{tool\_calls\_total}), execution time distributions (\texttt{tool\_execution\_time\_seconds}), and retry attempt tracking. These directly support RQ2 evaluation of tool success rates and retry recovery mechanisms.
    \item \textbf{Safety-Critical Metrics:} Crisis escalation counters (\texttt{crisis\_escalations\_total}), risk classification accuracy gauges (\texttt{safety\_triage\_accuracy}), and false negative rate tracking. These metrics are essential for RQ1 evaluation of Safety Triage Agent performance.
\end{itemize}

All metrics are scraped at 15-second intervals, providing high-resolution time-series data suitable for both real-time monitoring and post-hoc statistical analysis.

\paragraph{Grafana Dashboards.} A custom Grafana dashboard (\texttt{thesis-research-questions.json}) provides visual monitoring of RQ1-RQ4 metrics with color-coded pass/fail indicators against target thresholds (e.g., sensitivity $\geq$ 0.90, tool success rate $\geq$ 0.95). The dashboard organizes metrics into four collapsible rows corresponding to each research question, facilitating rapid assessment of system performance and identification of regressions during iterative development \cite{PLACEHOLDER_CITE_grafana}.

Dashboard panels include gauge charts for target threshold comparison, time-series graphs for trend analysis, and histogram visualizations for latency distribution analysis (p50, p90, p95, p99 percentiles).

\paragraph{Langfuse LLM Tracing.} All LLM invocations and agent state transitions are traced using Langfuse \cite{PLACEHOLDER_CITE_langfuse}, an open-source observability platform for LLM applications. Each agent execution generates a complete trace showing:
...
These traces provide detailed execution logs that enable debugging of agent reasoning failures, validation of orchestration correctness, and empirical evidence for RQ2 evaluation. The trace data forms the basis for analyzing state transition validity and identifying failure modes in the multi-agent coordination logic.

\begin{figure}[htbp]
    \centering
    \fbox{\textit{[PLACEHOLDER FIGURE: Monitoring architecture diagram showing Prometheus scraping backend /metrics endpoint, Grafana dashboard visualization with RQ1-RQ4 panels, and Langfuse trace collection from LangGraph agent executions. Arrows indicate data flow from backend to monitoring stack.]}}
    \caption{Observability infrastructure architecture supporting quantitative evaluation of research questions.}
    \label{fig:monitoring_architecture}
\end{figure}

This observability infrastructure is essential for the evaluation methodology described in Chapter~\ref{chap:evaluation}. It provides the instrumentation necessary to measure agent performance against quantitative targets and generate the empirical data required for statistical analysis. The metrics collected through this stack populate the evaluation database tables (Section~\ref{sec:data_persistence_evaluation}) and enable the automated calculation of research question metrics via the \texttt{calculate\_rq\_metrics.py} analysis script.

\subsection{Frontend Service: The UGM-AICare Web Application}

The frontend is a Next.js web application that serves as the human-computer interface for both students and administrative staff. It provides a conversational interface (\texttt{/aika} chat endpoint), user dashboard for tracking progress and journaling, and an admin panel for case management and insights visualization. The frontend communicates with the backend exclusively through the REST API, maintaining clean separation between presentation layer and business logic.

The detailed user experience design, interface implementation, and usability evaluation are beyond the scope of this thesis, which focuses on the multi-agent architecture evaluation. The frontend serves as a functional demonstration interface to validate that the agentic workflows can be successfully integrated into a complete application, but its performance characteristics and user experience quality are not subjects of formal evaluation in this work.
...
\subsubsection{Evaluation Database Schema}

Beyond the operational database tables (users, conversations, cases, appointments), the schema includes dedicated evaluation tables designed to support the research question methodology detailed in Chapter~\ref{chap:evaluation}. These tables capture ground-truth labels, agent predictions, human evaluations, and performance metrics required for quantitative analysis of the multi-agent framework.

Table~\ref{tab:evaluation_schema} summarizes the key evaluation tables and their correspondence to research questions.

\begin{table}[htbp]
    \centering
    \caption{Evaluation database schema summary.}
    \label{tab:evaluation_schema}
    \small
    \setlength{\tabcolsep}{3pt}
    \begin{tabular}{p{4.2cm}p{4.8cm}p{3.8cm}}
        \toprule
        \textbf{Table Name} & \textbf{Purpose} & \textbf{Research Question} \\
        \midrule
        \texttt{evaluation\_triage\_results} & Stores STA predictions vs. ground truth labels; enables confusion matrix calculation & RQ1: Safety accuracy, sensitivity, FNR \\
        \midrule
        \texttt{tool\_execution\_log} & Records all tool invocations with success/failure status, retry attempts, execution time & RQ2: Tool success rate, retry recovery \\
        \midrule
        \texttt{langgraph\_node\_execution} & Captures agent state transitions, node execution times, orchestration flow validity & RQ2: State correctness, agent latency \\
        \midrule
        \texttt{coach\_response\_evaluation} & Stores human rater scores (1-5 scale) for CBT adherence, empathy, appropriateness dimensions & RQ3: Therapeutic quality assessment \\
        \midrule
        \texttt{insights\_aggregates} & Anonymized aggregate statistics with group sizes; enables k-anonymity validation & RQ4: K-anonymity ($k \geq 5$), suppression rate \\
        \midrule
        \texttt{insights\_raw\_data} & Pre-aggregation conversation data with suppression flags and topic classifications & RQ4: JS divergence, topic accuracy \\
        \bottomrule
    \end{tabular}
\end{table}

\paragraph{RQ1 Evaluation Schema.} The \texttt{evaluation\_triage\_results} table stores each test case with fields for \texttt{ground\_truth\_risk\_level}, \texttt{predicted\_risk\_level}, \texttt{confidence\_score}, and \texttt{processing\_time\_ms}. This enables calculation of the confusion matrix (TP, TN, FP, FN) required to compute sensitivity, specificity, false negative rate, and precision metrics. The processing time field supports latency percentile analysis (p50, p95, p99).

\paragraph{RQ2 Evaluation Schema.} Two tables support orchestration evaluation: \texttt{tool\_execution\_log} tracks individual tool calls with fields for \texttt{tool\_name}, \texttt{status} (success/failure), \texttt{retry\_count}, and \texttt{execution\_time\_ms}. The \texttt{langgraph\_node\_execution} table captures agent state transitions with \texttt{node\_name}, \texttt{input\_state}, \texttt{output\_state}, and \texttt{duration\_ms}, enabling validation that all state transitions conform to the LangGraph specification and measurement of agent reasoning latency.

\paragraph{RQ3 Evaluation Schema.} The \texttt{coach\_response\_evaluation} table stores human expert ratings with separate integer fields (1-5 scale) for \texttt{cbt\_adherence\_score}, \texttt{empathy\_score}, and \texttt{appropriateness\_score}. Boolean fields \texttt{should\_refuse} and \texttt{did\_refuse} enable calculation of refusal accuracy. The \texttt{evaluator} field supports inter-rater reliability analysis when multiple experts rate the same responses.

\paragraph{RQ4 Evaluation Schema.} Privacy evaluation requires two tables: \texttt{insights\_aggregates} stores final anonymized statistics with \texttt{topic}, \texttt{risk\_category}, \texttt{user\_count} (for k-anonymity checking), and \texttt{aggregate\_value}. The \texttt{insights\_raw\_data} table contains pre-aggregation records with \texttt{user\_id}, \texttt{topic\_classification}, and \texttt{suppressed} boolean flag, enabling calculation of suppression rate and Jensen-Shannon divergence between observed and expected topic distributions.

\begin{figure}[htbp]
    \centering
    \fbox{\textit{[PLACEHOLDER FIGURE: Entity-Relationship Diagram showing evaluation tables (rectangular boxes) with primary keys underlined, foreign key relationships (arrows), and key fields listed. Operational tables (users, conversations) shown in light gray; evaluation tables (evaluation\_triage\_results, tool\_execution\_log, etc.) shown in blue. Arrows indicate relationships and data flow.]}}
    \caption{Evaluation database schema supporting RQ1-RQ4 quantitative analysis.}
    \label{fig:evaluation_erd}
\end{figure}

These evaluation tables enable the Python analytics script (\texttt{calculate\_rq\_metrics.py}, detailed in Chapter~\ref{chap:evaluation}) to compute statistical metrics (confusion matrices, percentile latencies, k-anonymity violations, JS divergence) required for quantitative evaluation. The schema design follows database normalization principles while optimizing for analytical query performance through appropriate indexing on frequently filtered columns (\texttt{ground\_truth\_risk\_level}, \texttt{status}, \texttt{evaluator}) \cite{PLACEHOLDER_CITE_database_design}.

The detailed logical structure, including complete field specifications and constraints, is available in the database migration files in the project repository but is omitted here to maintain focus on the architectural rationale and evaluation methodology.
