...
\begin{enumerate}
    ...
    \item \textbf{Independent Performance Optimization:} Each agent can employ task-appropriate LLM configurations. For instance, the STA prioritizes \textit{low latency} (using Gemini 2.0 Flash with temperature=0.1 for deterministic classification), while the SCA prioritizes \textit{response quality} (using Gemini 2.5 Pro with temperature=0.7 for natural, empathetic conversation). This optimization would be impossible with a monolithic architecture.
    
    \item \textbf{Domain-Specific Tool Access:} Tool-calling is scoped to agent roles. The STA has access to \code{escalate_crisis} and \code{log_risk_assessment} tools but \textit{cannot} schedule appointments or generate intervention plans---capabilities reserved for the SCA and Service Desk Agent (SDA). This enforces the principle of \textit{least privilege} and prevents unintended agent behaviors.
\end{enumerate}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{p{3.5cm}p{4.5cm}p{5.0cm}}
        ...
        \midrule
        Functional specialization & Four domain-specific agents (STA, SCA, SDA, IA) with role-scoped prompts and tools & \textbf{Monolithic LLM:} Single model handling all tasks. Rejected due to: (1) conflicting optimization objectives (speed vs. quality), (2) inability to isolate safety logic, (3) complex prompt engineering. \\
        \midrule
        ...
        \midrule
        Privacy-by-architecture & Strict data access boundaries: identified data (STA/SCA/SDA) vs. anonymized data (IA) & \textbf{Centralized data lake:} All agents access unified database. Rejected due to: (1) re-identification risk, (2) privacy breach exposure, (3) violates least-privilege principle. \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{Anonymized Database Access:} Read-only access to the \code{conversation_logs} table, from which all personally identifiable information (PII) has been redacted.
\end{itemize}
...
A\_{\text{SDA}} \rightarrow \text{END} & \text{if } \texttt{classify\_intent}(u) = \texttt{operational}
\end{cases}
...
\subsubsection{Support Coach Agent (SCA) Prompt Design}

The SCA's prompts are designed to deliver \textbf{empathetic, CBT-informed support} while maintaining strict ethical boundaries. Unlike the STA's classification-focused prompt, the SCA's prompt emphasizes \textit{conversational quality} and \textit{therapeutic alignment}. The system uses \textit{dynamic prompt selection} based on intervention type:

\begin{table}[htbp]
    \centering
    \caption{SCA prompt variants by intervention type.}
    \label{tab:sca_prompts}
    \small
    \setlength{\tabcolsep}{4pt}
...
An excerpt from the \code{calm_down} prompt illustrates the empathy-focused coaching style:

\begin{figure}[htbp]
\centering
\begin{lstlisting}[language=Python, basicstyle=\tiny\ttfamily, breaklines=true, frame=single, caption={Excerpt from SCA's anxiety management prompt.}, label={lst:sca_prompt}]
You are an expert mental health support coach specializing in anxiety and panic management.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{lll}
        ...
        \code{POST} & \code{/api/appointments} & Creates a new appointment with a counselor via the SDA. \\
...
\begin{table}[htbp]
    ...
    \begin{tabular}{p{3.5cm}p{2.0cm}p{4.5cm}p{2.5cm}}
        ...
        \midrule
        \multicolumn{4}{l}{\textit{SCA Outputs (intervention planning)}} \\
        \code{intervention_plan} & \code{Dict} & Generated intervention plan with steps and resources & SCA \\
        \code{intervention_type} & \code{str} & Plan type: "calm\_down"|"break\_down\_problem"|etc. & SCA \\
        \code{should_intervene} & \code{bool} & Flag indicating if intervention was created & SCA \\
        \code{intervention_plan_id} & \code{int} & Database ID of InterventionPlanRecord & SCA \\
        \midrule
        \multicolumn{4}{l}{\textit{SDA Outputs (case management)}} \\
        \code{case_id} & \code{int} & Database ID of created Case (if escalated) & SDA \\
        \code{case_created} & \code{bool} & Flag indicating case creation & SDA \\
        \code{assigned_counsellor_id} & \code{int} & ID of assigned counselor (if auto-assigned) & SDA \\
        \code{sla_breach_at} & \code{str} & ISO timestamp of SLA deadline & SDA \\
        \midrule
