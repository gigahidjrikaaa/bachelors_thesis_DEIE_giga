...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item \textbf{Safety Triage Agent (STA):} Runs asynchronously after the user becomes inactive or explicitly ends the chat. It reprocesses the entire conversation to confirm the final risk rating, generate the conversation-level STA assessment, and supply evidence for the compliance ledger.
    \end{itemize}
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        async_arrow/.style={-Latex, thick, dashed}
    ]
        ...
        \node[agent, at=(realtime_loop.center)] (aika) {Aika Meta-Agent};
        \node[agent, below=0.7cm of aika] (backgroundAgents) {Background Agents\\(STA, TCA, CMA)};
        
        \draw[arrow] (student) to[bend left=20] node[midway, above, font=\small] {Interaction} (aika);
        \draw[arrow] (aika) to[bend left=20] node[midway, below, font=\small] {Reply} (student);
        \draw[async_arrow] (aika) -- node[midway, right, font=\small] {Async dispatch} (backgroundAgents);
        \draw[async_arrow] (backgroundAgents) -- node[midway, left, font=\small, align=center] {Evidence \\ Plans} (aika);
        ...
        \draw[data_arrow] (backgroundAgents) -- (agg_data) node[midway, right, font=\small] {Fuels};
        \draw[data_arrow] (agg_data) -- (ia) node[midway, right, font=\small] {Informs};
        \draw[data_arrow] (ia) to[bend right=30] (aika) node[midway, left, font=\small] {Improves};
