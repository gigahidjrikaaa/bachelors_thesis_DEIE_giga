...
\begin{itemize}
    \item \textbf{Students:} As the primary users, students interact exclusively with the \textbf{Aika Meta-Agent} through the conversational interface (UGM-AICare's `/aika` page). Aika is the sole user-facing component; students never directly access the specialist agents (STA, TCA, CMA, IA), which operate transparently in the background. This unified interface provides a consistent conversational experience while internally orchestrating specialist agent invocations as needed.
    \item \textbf{University Staff/Counselors:} As the system's administrators and clinical supervisors, these stakeholders also interact primarily through Aika's conversational interface for analytics queries and case reviews, supplemented by a secure Admin Dashboard for detailed case management, escalation reviews, and system configuration. They can invoke the Insights Agent (IA) or Case Management Agent (CMA) through natural language requests to Aika (e.g., "show me this week's trending topics" routes to IA; "assign Case \#47 to Dr. Smith" routes to CMA).
    \item \textbf{The Agentic AI Backend:} This is the core computational engine of the framework. It hosts the five components of the Safety Agent Suite (four specialist agents plus the Aika Meta-Agent orchestrator), manages their stateful interactions via LangGraph, and serves as the central hub for all data processing, logic execution, and communication with external services and databases. Critically, only Aika is exposed to user-facing interfaces; specialist agents operate as internal microservices invoked exclusively through Aika's orchestration logic.
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Modular Evolution:} When safety triage logic requires refinement (e.g., adjusting crisis keywords or classification thresholds), changes are isolated to the STA module without risk of inadvertently affecting the TCA's therapeutic coaching behavior. This modularity accelerates development velocity and reduces regression risk \cite{burguillo2017multiagentsystems}.
...
\label{sec:agent_role_segregation}

A critical design principle in the Safety Agent Suite is \textbf{strict role segregation} to prevent functional overlap and ensure clear accountability boundaries. This segregation addresses common pitfalls in multi-agent systems where ambiguous role definitions can lead to redundant operations, inconsistent behavior, and complex debugging scenarios \cite{wooldridge2009introductionmas}. The nomenclature change from ``Support Coach Agent'' to ``Therapeutic Coach Agent'' and ``Service Desk Agent'' to ``Case Management Agent'' reinforces this segregation by making role boundaries linguistically explicit.

\paragraph{Therapeutic Coach Agent (TCA)}

\noindent \textbf{Designated Responsibilities:}
\begin{itemize}
    \item CBT-informed therapeutic coaching and emotional support
    \item Intervention plan generation with structured coping strategies
    \item Therapeutic exercise guidance (cognitive restructuring, behavioral activation)
    \item Progress tracking for wellness goals and mood patterns
    \item Resource recommendations (self-help modules, psychoeducation materials)
\end{itemize}

\noindent \textbf{Explicitly Excluded Capabilities:}
\begin{itemize}
    \item Crisis case creation or counselor escalation workflows
    \item Appointment scheduling or SLA tracking
    \item Administrative notifications to clinical staff
    \item Any operational/procedural tasks beyond therapeutic support
\end{itemize}

\noindent \textbf{Design Rationale:} The TCA maintains exclusive focus on therapeutic intervention without dilution from operational concerns. This enables temperature=0.7 configuration for empathetic, human-like conversation generation while ensuring the agent never attempts administrative actions it lacks authority to perform. The prompt engineering explicitly instructs: ``If user requests appointment scheduling or administrative help, respond: 'I'll connect you with our case management team for that.' Do NOT attempt scheduling yourself.''

\paragraph{Case Management Agent (CMA)y}

\noindent \textbf{Designated Responsibilities:}
\begin{itemize}
    \item Crisis case creation with structured risk documentation
    \item Appointment scheduling with counselor availability management
    \item Counselor assignment based on specialization and workload
    \item SLA calculation, breach prediction, and follow-up tracking
    \item Administrative notifications (counselor alerts, status updates)
\end{itemize}

\noindent \textbf{Explicitly Excluded Capabilities:}
\begin{itemize}
    \item Therapeutic coaching or CBT-based interventions
    \item Emotional support conversations or empathy-focused dialogue
    \item Intervention plan generation or coping strategy recommendations
    \item Any content requiring clinical/therapeutic expertise
\end{itemize}

\textbf{Design Rationale:} The CMA focuses on deterministic operational workflows requiring temperature=0.3 for consistent structured outputs (case IDs, SLA timestamps, counselor assignments). Preventing therapeutic content generation avoids boundary violations where a non-therapeutic agent provides clinical advice, which could undermine treatment integrity and violate counselor oversight protocols.

\paragraph{Enforcement Mechanisms}

This segregation is enforced through three complementary mechanisms:
\begin{enumerate}
    \item \textbf{Tool Access Control:} TCA has access to \code{generate\_intervention\_plan} and \code{recommend\_resources} tools but \textit{cannot} invoke \code{create\_case} or \code{schedule\_appointment}. Conversely, CMA has access to administrative tools but \textit{cannot} call \code{generate\_intervention\_plan}. Tool schemas are defined in separate modules with role-scoped imports.
    
    \item \textbf{Prompt Engineering with Refusal Policies:} System prompts explicitly define role boundaries with refusal instructions. When a TCA receives an administrative request, it responds: ``Let me connect you with our case management team for that'' and triggers Aika to route the next message to CMA. Similarly, CMA prompts prohibit therapeutic advice: ``I handle scheduling and case management. For emotional support, I'll reconnect you with your therapeutic coach.''
    
    \item \textbf{Orchestration Routing Invariants:} The Aika Meta-Agent enforces deterministic routing based on intent classification. Therapeutic intents (\code{emotional\_support}, \code{coping\_strategies}) route exclusively to TCA; operational intents (\code{schedule\_appointment}, \code{escalate\_crisis}) route exclusively to CMA. This prevents agents from receiving out-of-scope requests that could trigger inappropriate responses.
\end{enumerate}
...
\begin{itemize}
    \item \textbf{Reactive Loop (STA → TCA → CMA):} Triggered by incoming student messages, this loop provides real-time crisis detection and synchronous therapeutic support. The workflow executes in seconds: STA classifies risk (150-300ms), TCA generates intervention plan (1-2s), and CMA creates case record if escalation is required (200ms).
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        userfacing/.style={entity, fill=ugmGold!20, draw=ugmGold, line width=1.5pt},
        ...
        bgagent/.style={rectangle, rounded corners=3pt, draw=ugmBlue!40, fill=ugmBlue!8, minimum width=2.8cm, minimum height=0.75cm, font=\scriptsize, align=center, dashed},
        arrow/.style={-Latex, thick, ugmBlue},
        userarrow/.style={-Latex, very thick, ugmGold},
        dashedarrow/.style={-Latex, thick, ugmBlue, dashed}
    ]
        \node[external] (student) {Student\\\footnotesize UGM-AICare App};
        \node[entity, right=3.8cm of student, minimum width=4.6cm, minimum height=6.5cm] (backend) {};
        \node at (backend.north) [yshift=-0.35cm, font=\bfseries\small, text=ugmBlue] {Safety Agent Suite Backend};
        
        % Highlight Aika as USER-FACING
        \node[userfacing] (aika) at ([yshift=1.65cm]backend.center) {\textbf{Aika Meta-Agent}\\\textit{SOLE USER-FACING}};
        
        % Background agents (dashed borders)
        \node[bgagent] (sta) at ([yshift=0.65cm]backend.center) {Safety Triage (STA)\\\textit{Background Tier 2}};
        \node[bgagent, below=0.15cm of sta] (tca) {Therapeutic Coach (TCA)\\\textit{Background Plans}};
        \node[bgagent, below=0.15cm of tca] (cma) {Case Mgmt (CMA)\\\textit{Conditional via Aika}};
        \node[bgagent, below=0.15cm of cma] (ia) {Insights (IA)\\\textit{Conditional via Aika}};
        
        \node[external, right=3.6cm of backend] (staff) {University Staff\\\footnotesize Admin Dashboard};
        \node[datastore, below=2.2cm of backend] (dataplatform) {Data Platform\\\footnotesize Encrypted storage \& analytics};
        \node[external, above=2.2cm of backend] (llm) {LLM Service\\\footnotesize (Gemini API)};

        % User arrows emphasize Aika as interface
        \draw[userarrow] (student) -- node[above, font=\footnotesize, fill=white]{\textbf{Messages (to Aika only)}} (backend);
        \draw[userarrow] (backend) -- node[above, font=\footnotesize, fill=white]{\textbf{Aika responses}} (student);
        \draw[userarrow] (backend) -- node[above, font=\footnotesize, fill=white]{\textbf{Aika + Analytics}} (staff);
        \draw[dashedarrow] (staff) -- node[below, font=\footnotesize]{Config, reviews} (backend);
        \draw[arrow] (backend) -- node[right, font=\footnotesize]{Anonymized logs} (dataplatform);
        \draw[dashedarrow] (dataplatform) -- node[left, font=\footnotesize]{Aggregated reports} (backend);
        \draw[arrow] (backend) -- node[right, font=\footnotesize]{Prompts, JSON schema} (llm);
        \draw[dashedarrow] (llm) -- node[left, font=\footnotesize]{JSON responses} (backend);
    \end{tikzpicture}}
    \caption{High-level context of the Safety Agent Suite emphasizing Aika Meta-Agent as the sole user-facing component. Students and staff interact exclusively with Aika's conversational interface (gold highlight); specialist agents (STA, TCA, CMA, IA) operate as internal background services (dashed borders) invoked conditionally through Aika's orchestration logic. This architecture ensures consistent user experience while enabling modular specialist intelligence.}
    \label{fig:context_diagram}
\end{figure}

\subsection{Aika's Unified JSON Response Structure}
\label{subsec:aika_json_response}

A critical architectural decision in the Safety Agent Suite is Aika Meta-Agent's use of a single, structured JSON response from the Gemini API that integrates both conversational content and embedded risk assessment (Tier 1). This design consolidates what would traditionally require multiple sequential API calls into a single sub-second interaction, enabling simultaneous dialogue generation and immediate safety screening without introducing latency bottlenecks.

\subsubsection{Response Schema}

When a user sends a message, Aika invokes the Gemini 2.5 Flash model with a specialized system prompt that instructs the model to return a JSON object containing the following fields:

\begin{itemize}
    \item \textbf{\code{reply}} (string): The conversational response to display to the user. This text embodies Aika's personality (warm, empathetic, culturally appropriate for Indonesian university students) and addresses the user's query directly. For example: \texttt{"Halo! Saya Aika, asisten kesehatan mental UGM. Ada yang bisa saya bantu hari ini?"}
    
    \item \textbf{\code{intent}} (string): Classified intent of the user's message. Values include: \code{emotional\_support}, \code{crisis}, \code{casual\_chat}, \code{appointment\_request}, \code{analytics\_query}, \code{case\_management}, \code{unknown}. This classification drives Aika's routing decisions to specialist agents.
    
    \item \textbf{\code{intent\_confidence}} (float, 0.0--1.0): Confidence score for the intent classification. Low-confidence classifications ($<0.6$) trigger fallback behaviors or request clarification from the user.
    
    \item \textbf{\code{immediate\_risk}} (enum): Tier 1 risk assessment performed inline during response generation. Values: \code{none}, \code{low}, \code{moderate}, \code{high}, \code{critical}. This field embeds safety screening directly into every response, eliminating the need for a separate STA invocation for per-message screening.
    
    \item \textbf{\code{crisis\_keywords}} (array of strings): List of crisis-related keywords detected in the user's message (e.g., \code{["bunuh diri", "ingin mati"]}). Empty array if no crisis indicators found. Used for audit trails and counselor handoff context.
    
    \item \textbf{\code{risk\_reasoning}} (string): Brief 1--2 sentence explanation of the risk assessment (e.g., \code{"Explicit suicide plan with method and timeframe detected"}). Provides transparency for counselor review and debugging.
    
    \item \textbf{\code{needs\_agents}} (boolean): Decision flag indicating whether specialist agents (STA, TCA, CMA, IA) need to be invoked. \code{false} indicates Aika can respond directly without specialist consultation (e.g., for simple greetings); \code{true} triggers specialist agent routing.
    
    \item \textbf{\code{reasoning}} (string): Explanation of why \code{needs\_agents} is \code{true} or \code{false}, documenting Aika's decision-making logic for observability and debugging.
    
    \item \textbf{\code{suggested\_response}} (string, optional): If \code{needs\_agents} is \code{false}, this field contains the complete response to send directly to the user, bypassing specialist agent invocation. This optimization reduces latency for simple interactions (e.g., greetings, FAQ answers).
\end{itemize}

\subsubsection{Architectural Benefits}

This unified JSON structure provides several critical advantages:

\begin{enumerate}
    \item \textbf{Latency Optimization}: By consolidating conversational response generation and risk assessment into a single LLM call, the system achieves sub-300ms response times for direct Aika replies, compared to 500--800ms for sequential agent invocations. This is critical for maintaining conversational fluidity in a mental health support context where perceived responsiveness impacts user trust.
    
    \item \textbf{Two-Tier Risk Monitoring Integration}: The \code{immediate\_risk} field implements Tier 1 (per-message) risk screening without requiring a separate STA invocation for every message. Tier 2 (conversation-level) analysis by STA is deferred until conversation end, reducing API costs by approximately 90\% (11 calls vs. 20 per 10-message conversation) while maintaining comprehensive safety coverage as documented in Section~\ref{subsec:two_tier_risk}.
    
    \item \textbf{Observability and Auditability}: The \code{reasoning}, \code{risk\_reasoning}, and \code{crisis\_keywords} fields provide explicit provenance for every routing and safety decision, enabling post-hoc analysis of agent behavior, false negative investigations, and counselor handoff transparency. This addresses ethical requirements for explainable AI in healthcare contexts \cite{FIND_CITATION_PLACEHOLDER}.
    
    \item \textbf{Conditional Agent Invocation}: The \code{needs\_agents} flag enables efficient resource utilization by bypassing specialist agent orchestration for simple queries (e.g., ``Hello'', ``What is CBT?''), reducing backend compute costs and improving system scalability under high concurrent user loads.
\end{enumerate}

\subsubsection{Example JSON Response}

A typical JSON response for a student expressing moderate stress might look like:

\begin{lstlisting}[language=json,caption={Example Aika JSON response for moderate stress scenario}]
{
  "reply": "Saya mengerti kamu merasa tertekan dengan ujian yang akan datang. Wajar banget kok merasa cemas. Yuk, kita coba teknik grounding bersama untuk menenangkan pikiran.",
  "intent": "emotional_support",
  "intent_confidence": 0.87,
  "immediate_risk": "low",
  "crisis_keywords": [],
  "risk_reasoning": "Student expresses exam stress without crisis indicators",
  "needs_agents": true,
...
\begin{lstlisting}[language=json,caption={Example Aika JSON response for casual greeting}]
{
  "reply": "Halo! Saya Aika, asisten kesehatan mental UGM. Saya di sini untuk mendengarkan dan membantu kamu. Ada yang ingin kamu ceritakan?",
  "intent": "casual_chat",
  "intent_confidence": 0.95,
  "immediate_risk": "none",
  "crisis_keywords": [],
  "risk_reasoning": "No distress signals detected",
  "needs_agents": false,
  "reasoning": "Simple greeting requires no specialist agent consultation",
  "suggested_response": "Halo! Saya Aika, asisten kesehatan mental UGM. Saya di sini untuk mendengarkan dan membantu kamu. Ada yang ingin kamu ceritakan?"
}
\end{lstlisting}
...
\begin{equation}
\text{Level 2 (Specialist Layer)}: \quad \mathcal{S} = \{(A_i, \mathcal{D}_i, f_i) \mid i \in \{\text{STA, TCA, CMA, IA}\}\}
...
where:
\begin{itemize}
    \item $\pi_{\text{safety}}(u)$ performs immediate risk screening (Tier 1) via Aika's JSON output, auto-escalating to $\{A_{\text{CMA}}\}$ for high/critical risk
    ...
    \item At conversation end, $\{A_{\text{STA}}\}$ performs comprehensive Tier 2 analysis with full conversation context
\end{itemize}
...
\begin{figure}[h]
    ...
    \begin{tikzpicture}[
        ...
        % Meta-Agent Layer (highlighted as USER-FACING)
        \node[metanode, below=2cm of admin] (aika) {
            \textbf{Aika Meta-Agent}\\
            {\scriptsize\textit{SOLE USER-FACING INTERFACE}}\\
            {\scriptsize JSON: \{reply, immediate\_risk, ...\}}
        };
        
        % Layer label for meta-agent
        \node[left=0.3cm of aika, font=\scriptsize, text width=1.5cm, align=right] (metalabel) {\textit{User-\\Facing\\Layer}};
        
        % Specialist Agent Layer (BACKGROUND - dashed borders)
        \node[agentnode, below left=2.5cm and 3.5cm of aika] (sta) {
            \textbf{STA}\\
            {\scriptsize Tier 2 (conv. end)}
        };
        \node[agentnode, right=0.8cm of sta] (tca) {
            \textbf{TCA}\\
            {\scriptsize Background plans}
        };
        \node[agentnode, right=0.8cm of tca] (cma) {
            \textbf{CMA}\\
            {\scriptsize Crisis escalation}
        };
        \node[agentnode, right=0.8cm of cma] (ia) {
            \textbf{IA}\\
            {\scriptsize On-demand analytics}
        };
        
        % Layer label for specialist agents
        \node[left=0.3cm of sta, font=\scriptsize, text width=1.5cm, align=right] (agentlabel) {\textit{Background\\Services\\(internal)}};
        
        % Input arrows from user roles to meta-agent (BOLD - user-facing)
        ...
        % Routing arrows from meta-agent to specialists (DASHED - internal)
        \draw[routearrow] (aika.south) -- ++(-3.5,-1.0) -- node[edgelabel, above left, pos=0.7] {\scriptsize conv. end} (sta.north);
        \draw[routearrow] (aika.south) -- ++(-1.3,-1.0) -- node[edgelabel, above left, pos=0.6] {\scriptsize dashboard} (tca.north);
        \draw[routearrow] (aika.south) -- ++(1.3,-1.0) -- node[edgelabel, above right, pos=0.6] {\scriptsize if crisis} (cma.north);
        \draw[routearrow] (aika.south) -- ++(3.5,-1.0) -- node[edgelabel, above right, pos=0.7] {\scriptsize if admin query} (ia.north);
        
        % Response aggregation arrows (dashed)
        \draw[backarrow] (sta.north) -- ++(0,0.6) -| node[edgelabel, above, pos=0.12] {\scriptsize proactive recs} (aika.south);
        \draw[backarrow] (tca.north) -- ++(0,0.4) -| node[edgelabel, above, pos=0.15] {\scriptsize plans} (aika.south);
        \draw[backarrow] (cma.north) -- ++(0,0.4) -| node[edgelabel, above, pos=0.15] {\scriptsize case ID} (aika.south);
        \draw[backarrow] (ia.north) -- ++(0,0.6) -| node[edgelabel, above, pos=0.12] {\scriptsize analytics} (aika.south);
        ...
        \node[below=3.2cm of aika, font=\scriptsize, text width=12cm, align=left] (legend) {
            ...
            \textbf{Bold box} = user-facing interface (Aika only); 
            \textbf{dashed boxes} = background services invoked conditionally through Aika. 
            Solid arrows = user requests; dashed arrows = internal routing/responses.
        };
    \end{tikzpicture}
    \caption{Revised hierarchical architecture emphasizing Aika Meta-Agent as the sole user-facing component (highlighted in yellow). Students and staff interact exclusively with Aika, which performs immediate Tier 1 risk screening via structured JSON responses. Specialist agents (dashed borders) operate as internal background services: STA analyzes conversations at end (Tier 2), TCA generates dashboard intervention plans asynchronously, CMA is invoked for crisis escalation, and IA is invoked for admin analytics queries. This architecture ensures a unified conversational experience while enabling modular specialist intelligence.}
...
\begin{enumerate}
    ...
    \item \textbf{Role-Based Access Control:} The routing constraints $\pi_{\text{role}}(r)$ enforce privilege separation: students cannot access administrative analytics ($A_{\text{IA}}$) or case management ($A_{\text{CMA}}$), preventing unauthorized data exposure.
    ...
    \item \textbf{Modularity and Maintainability:} Changes to specialist agent logic (e.g., updating CBT prompts in $f_{\text{TCA}}$) do not require modifications to $\Phi_{\text{Aika}}$, as long as input/output schemas remain stable.
\end{enumerate}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{p{3.0cm}p{5.0cm}p{4.5cm}}
        ...
        \midrule
        \code{user_context} & Retrieve user profile, preferences, mental health summary & TCA, Aika (student role) \\
        \code{progress_tracking} & Wellness state, goal tracking, mood logging & TCA, Aika (student role) \\
        \code{conversation} & Conversation history, search past messages & TCA, CMA, Aika \\
        \code{safety} & Risk assessment history, active cases, crisis resources & STA, TCA, CMA \\
        \code{intervention} & CBT modules, intervention plans, treatment resources & TCA \\
        \code{case_management} & Create cases, assign counselors, update case status & CMA, Aika (counselor/admin roles) \\
...
Consider a scenario where a student says, \textit{"Aku stres banget dengan tugas akhir semester"} (I'm so stressed about final semester assignments). The TCA's tool calling flow proceeds as follows:

\begin{enumerate}
    \item \textbf{Context Analysis:} The TCA receives the message along with conversation history and user profile from the shared state.
...
\begin{enumerate}
    ...
    \item \textbf{Synthetic Testing:} Evaluate prompts against 50 synthetic crisis scenarios (detailed in Chapter~\ref{chap:evaluation}) to measure classification accuracy (STA) and response quality (TCA).
    
    \item \textbf{Failure Analysis:} Identify edge cases where agents produced undesired behaviors (e.g., STA under-classifying implicit crisis signals, TCA providing overly generic advice).
...
The core state schema, \code{SafetyAgentState}, defines the data contract for the STA $\rightarrow$ TCA $\rightarrow$ CMA orchestration workflow. Table~\ref{tab:state_schema} presents the key fields grouped by functional category.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{p{3.5cm}p{2.0cm}p{4.5cm}p{2.5cm}}
        ...
        \code{next_step} & \code{str} & Routing decision: "tca"|"cma"|"end" & STA \\
...
\begin{enumerate}
    ...
    \item \textbf{Agent-Specific State Extensions:} Agents can define subclasses of \code{SafetyAgentState} with additional fields. For example, \code{CMAState} extends the base schema with \code{assigned_to} and \code{assignment_reason} fields used internally by the CMA but ignored by upstream agents.
\end{enumerate}
